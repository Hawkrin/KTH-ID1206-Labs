...
#define NO_OF_TLB_ROWS 16        // Number of rows in the TLB
...
int main() {
    ...
    int 
        ...
        TLB[NO_OF_TLB_ROWS][NO_OF_TLB_COLUMNS];

    memset(TLB, -1, sizeof(TLB[0][0]) * NO_OF_TLB_ROWS * NO_OF_TLB_COLUMNS);
    ...
    while(fgets(ch, 10, address_file)) {
        ...
        TLB_hit = false;
        for (int i = 0; i < NO_OF_TLB_ROWS; i++) {
        ...
        if(page_table[page_number]!= -1) {
            ...
            TLB[TLB_counter % NO_OF_TLB_ROWS][0] = page_number;
            TLB[TLB_counter % NO_OF_TLB_ROWS][1] = physical_address;
            TLB_counter++;
        }
